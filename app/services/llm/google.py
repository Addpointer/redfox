...
def _config(
  ...
  """Add important config to the google genai config for use"""
  from google.genai import types
...
def _to_response(result: object, model: str, latency_ms) -> LLMResponse:
...
class GoogleProvider(LLMProvider):
  ...
  Google enterprise is what is being used for the provider
  """
  def __init__(
    self,
    *,
    ...
    if not google_project_id:
      raise LLMConfigError(
        "Google project_id is missing. Set GOOGLE_PROJECT_ID "
        "in the environment."
      )

    if not google_cloud_location:
      raise LLMConfigError(
        "Google cloud location is missing. Set GOOGLE_CLOUD_LOCATION "
        "in the environment"
      )
  ...
  def chat(
    ...
    *,
    ...
    """Generate a response according to a prompt"""
    if not messages:
    ...
    return _to_response(result, model or self._model, latency_ms)
  ...
  async def achat(
    ...
    *,
    ...
    """Generate an async response according to the prompt"""
    if not messages:
      raise ValueError("messages must not be empty")
    contents, system_instruction = _contents_and_system(messages)
    config = _config(
  ...
  def stream(
    ...
    *,
    ...
    """Generate a streaming response according to the prompt"""
    if not messages:
      raise ValueError("messages must not be empty")
    try:
      chunks = self._client().models.generate_content_stream(
        **self._stream_kwargs(messages, model or self._model, temperature, max_tokens)
      )
      for chunk in chunks:
  ...
  async def astream(
    ...
    *,
    ...
    """Generate an async streaming response according to the prompt"""
    if not messages:
      raise ValueError("messages must not be empty")
    try:
      chunks = self._client().aio.models.generate_content_stream(
        **self._stream_kwargs(messages, model or self._model, temperature, max_tokens)
      )
      for chunk in chunks:
        delta = getattr(chunk, "text", None) or ""
